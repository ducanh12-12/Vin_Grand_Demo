...
func parseEnv() Env {
	...
	return Env{
		...
		DBconfig: DBconf{
			...
			Username:        WithDefault("APP_DB_USERNAME", "postgres"),
			Password:        WithDefault("APP_DB_PASSWORD", "postgres"),
			DBname:          WithDefault("APP_DB_NAME", "vin_demo"),
			DBbackend:       WithDefault("APP_DB_BACKEND", "postgres"),
